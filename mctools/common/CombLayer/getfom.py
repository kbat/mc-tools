...
def main():
    ...
    parser.add_argument('-axis', dest='axis', type=int, default=-1, help='axis to project THnSparse on. If<0, the absolute value of BIN must be specified (see next argument). This option make sence only for THnSparse objects.')
    parser.add_argument('-bin', dest='bin', type=str, default="", help='bin number to use as the figure of merit. Plus-sign-separated list of bins is allowed: the result will be the sum over all bins. Hint for the MCNP case: the bin number is the serial number of the bin in the outp file provided that no FQ print hierarhy card was used. If used, AXIS must be set to a negative integer number.')
...
#    parser.add_argument('-select', dest='select', default=[], nargs='+', help='selection') # -select "cut1" "cut2"
    ...
    if args.bin == "":
            if args.axis >= 0:
                    raise Exception("BIN value must be specified with positive AXIS")
            elif args.axis < 0:
                    raise Exception("Absolute BIN value is needed with negative AXIS")

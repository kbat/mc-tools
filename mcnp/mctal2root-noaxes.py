...
#    arguments = parser.parse_args()
    ...
    good_tallies = [2] # list of 'good' tally types - to be saved in the ROOT file
    fname_in = sys.argv[1]
    if len(sys.argv) == 3:
    ...
    for line in file_in.readlines(): # memory more efficient without readlines
        if kod is None:
...
#            if verbose: print("number of tallies in the problem: %d" % ntal)
        ...
        if ntal and not tally and words[0] != 'tally': # list of tally numbers follows
            for w in words:
                ntals.append(int(w))
            if verbose:
